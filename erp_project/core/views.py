...
def login_view(request):
    if request.method == "POST":
        ...
        if not username or not password:
            return render(
                request, "core/login.html", {"error": "Please enter all fields"}
            )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not new_password:
                if (
                    profile.email == email
                    and profile.security_question == question
                    and profile.security_answer.lower() == answer.lower()
                ):
                    return render(
                        request,
            ...
            return render(
                request, "core/forgot_password.html", {"error": "User not found"}
            )
...
def add_product(request):
    if request.method == "POST":
        ...
        if not all([name, price, quantity]):
            return render(
                request, "core/add_product.html", {"error": "All fields required"}
            )

        Product.objects.create(
            user=request.user, name=name, price=price, quantity=quantity
        )
...
def add_customer(request):
    if request.method == "POST":
        ...
        if not all([name, email, phone]):
            return render(
                request, "core/add_customer.html", {"error": "All fields required"}
            )

        Customer.objects.create(
            user=request.user, name=name, email=email, phone=phone, address=address
        )
...
def create_invoice(request):
    ...
    if request.method == "POST":
        ...
        invoice = Invoice.objects.create(
            user=request.user, customer=customer, total_amount=0
        )
        total = 0
        ...
        for i in range(len(product_ids)):
            ...
            InvoiceItem.objects.create(
                invoice=invoice, product=product, quantity=qty, price=product.price
            )
...
def invoice_view(request, id):
    ...
    return render(
        request, "core/invoice_view.html", {"invoice": invoice, "items": items}
    )
...
def import_backup(request):
    if request.method == "POST":
        ...
        if not file:
            return render(
                request, "core/import_backup.html", {"error": "Please upload a file"}
            )
